...
class RefreshQuotesTest extends TestCase
{
    ...
    {
        $response = $this->json('GET', route('quotes.refresh'));
    ...
    {
        $response = $this->json('GET', route('quotes.refresh', ['token' => 'invalid']));
        $response->assertStatus(401);
    ...
    public function test_it_fails_if_the_token_is_expired(): void
    {
        ...
        $response = $this->json('GET', route('quotes.refresh', ['token' => $token]));
        $response->assertStatus(401);
